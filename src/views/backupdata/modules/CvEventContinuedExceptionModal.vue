  ...
  <j-modal
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="事件出现次数" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="['qty']" placeholder="请输入事件出现次数" style="width: 100%"/>
        </a-form-item>
        ...
        <a-form-item label="重要性" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['severity']" placeholder="请输入重要性"></a-input>
        </a-form-item>
...
<script>
  ...
  export default {
    name: "CvEventContinuedExceptionModal",
    components: {
      JDate,
      JDictSelectTag,
    },
    data () {
      return {
        ...
        url: {
          add: "/cvEventContinuedException/add",
          edit: "/cvEventContinuedException/edit",
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'domain','eventCode','qty','details','triggerTime','handled','severity'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'domain','eventCode','qty','details','triggerTime','handled','severity'))
      },
