  ...
  export default {
    ...
    methods: {
      init(){
        ...
        var params = {id:this.formData.vars.apply_no};//查询条件
        getAction(this.url.queryByTripApplyNo,params).then((res)=>{
          if(res.success){
            console.log("获取流程节点信息",res);
            this.edit (res.result);
          }else{
            this.edit({tripApplyNo:this.formData.vars.apply_no});
          }
        });
        var params2 = {tripApplyNo:this.formData.vars.apply_no};//查询条件
