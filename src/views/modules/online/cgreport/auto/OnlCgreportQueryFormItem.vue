<template>
  ...
  <a-form-item v-else-if="item.view=='Datetime' || item.view=='datetime'" :label="item.label">
    ...
    <template v-else>
      <div>
        <j-date placeholder="请选择开始时间" :show-time="true" date-format="YYYY-MM-DD HH:mm:ss" style="width:calc(50% - 15px)" v-model="queryParam[item.field+'_begin']" ></j-date>
        <span class="group-query-strig">~</span>
        <j-date placeholder="请选择开始时间" :show-time="true" date-format="YYYY-MM-DD HH:mm:ss" style="width:calc(50% - 15px)" v-model="queryParam[item.field+'_end']"></j-date>
      </div>
  ...
  <a-form-item v-else :label="item.label">
    <template v-if="group_mode===item.mode">
      <a-input :placeholder=" '请输入开始'+item.label " v-model="queryParam[item.field+'_begin']" style="width:calc(50% - 15px) !important;"></a-input>
      <span class="group-query-strig">~</span>
      <a-input :placeholder=" '请输入结束'+item.label " v-model="queryParam[item.field+'_end']" style="width:calc(50% - 15px) !important;"></a-input>
    </template>
    <template v-else>
